...
def start(message):
    bot.send_message(message.chat.id, 'Чтобы воспользоваться калькулятор, пропиши команду /calclater')
...
def calclater(call):
    ...
    if value != old_value: 
        if value == '':
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id, text='Калькулятор', reply_markup=markup)
        else:
